module Ylmish.Adaptive.Spike

// =============================================================================
// Plan 0006 — Step 1: characterize FSharp.Data.Adaptive's reassignment deltas.
//
// THE DECISIVE EXPERIMENT. Option A ("lean into Adaptive: feed its
// IndexListDelta straight to Yjs as element-wise ops") is only viable if, when
// adaptify's `Update` assigns a *whole fresh* immutable collection, Adaptive
// emits a MINIMAL, IDENTITY-STABLE delta — not a clear-all + add-all. This spike
// measures exactly that, and the same numbers price Option B (how much work
// Adaptive is actually doing for us).
//
...
//     element gets a new Index), so it always churns — directly relevant to the
//     "moves orphan nested state" risk.
//
// We MEASURE these (recording observed op counts, per the plan's discipline) and
// pin them so the finding can't silently drift.
// =============================================================================
...
let tests = testList "Adaptive reassignment (0006 Step 1)" [
    ...
    // Adaptive degrades to clear-all + add-all — Option A buys nothing over LWW.

    test "rebuild via IndexList.ofList (fresh Indices) → clear-all + add-all, NOT minimal" {
    ...
    test "reorder has NO identity-preserving form → churns (relevant to move/nested state)" {
    ...
    test "rebuilt list of submodels reuses inner adaptive objects POSITIONALLY, not by id" {
