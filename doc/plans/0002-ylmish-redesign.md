...
- **Decode direction.** One doc-level `'update'` hook; transactions carrying our own origin token are ignored; every other content-changing transaction produces exactly one decode → one Elmish `Set` dispatch (U6, U14). Custom elements ride the same read (L6). Decode failures go to `OnError` and leave the current model in place — a malformed or newer-versioned doc can't crash the loop.
- **The suppression window (added at Step 7).** Folding a decoded remote model back into the adaptive model is indistinguishable, by content alone, from a local edit — without countermeasures the encode direction re-splices remote text and duplicates remote list items (the reference branch's read-back hazard, L4, reproduced by the north stars on first run). `withYlmish` therefore folds every `Set` in under the attachment's suppression window: the encode direction realigns its internal trackers (text `last`, map child subscriptions, option attach state) but writes nothing, because the doc already holds exactly that content. Maps happened to survive without it (per-key LWW restamps are idempotent); text and lists did not — which is why the window is structural, not per-kind patching.
...
*Post-plan (executed 2026-07-12, at Nick's direction):* the example's `Note : Text` moved INTO the todo item — the realistic shape (a collaborative note per entity). That exercised text-inside-a-replaced-keyed-item for the first time and exposed the same class of gap act 4 found for registers: the item-replacement flush skipped non-fresh `Y.Text`s entirely, so note edits never reached the doc. Fix: `Binding.unflushedSplices` — the not-yet-flushed intents are a suffix of the new value's pending list, so replay the shortest suffix whose result matches the content (affix-diff fallback), keeping interleaving intent. Pinned at the example level ("concurrent edits to the same todo's note interleave"); demo act 2 restaged onto the todo's note; README/guides quotes and transcript re-synced. 110 passing.
...
- **156 passing, 0 pending, 0 failing — all three north stars green:** "concurrent Text edits converge interleaved across two withYlmish programs" (the #83 headline: both models read `"oh, hello world"`), "keyed-map concurrent adds both survive", "unknown keys survive a full withYlmish session". The two-process IPC demo (`npm run demo`) runs and converges over the v2 runtime unchanged.
- **The old world is gone:** `Y.Doc.materialize`/`dematerialize` and their helpers (~150 lines of `Y.fs`), the v1 `withYlmish` + `YlmishOptions` and ~330 lines of commented-out sketches (`Program.fs` went from ~490 lines to 147), the v1 `Y.Doc.fs` test file (568 lines), and the harness's materialize bridge + calibration tests (their red-flip duty is recorded here; the binding-vs-oracle check lives in the Binding tests). Zero code references to materialize remain. The v1 codec (`Ylmish.Adaptive.Codec`) and `Y.fs`'s attach plumbing REMAIN — the `Delta` module is the binding's list machinery and the rest is the internal layer with its own passing tests; scheduling their pruning is housekeeping, not Step 7 scope. Surfaced as an interpretation.
- **The v1 Program tests were not ported — they were replaced.** v1 pinned materialize semantics (eager persistence of the init model); v2's contract is decode-empty-=-init with NO eager writes, so "persists initial value"-style tests are wrong by design now. The new suite pins: init-on-empty writes nothing; init-on-existing restores through the decoder (`ask` keeping app-only fields); **one Elmish update = one origin-tagged Y transaction even across multiple fields** (the write-side U14, via nesting the adaptive transact inside `Y.transact` tagged with the attachment origin — Yjs joins nested transactions); remote Set + no echo + app-only fields never sync; OnError on decode failure with a surviving loop; unknown-key preservation; and the **dual-key migration recipe as a passing test built from plain combinators** (v1 and v2 schemas share a doc; v2 reads new-or-old and dual-writes; v1 keeps working; neither destroys the other's keys).
- **The step's real discovery — the suppression window** (now in the design section): the north stars failed on first run with `"oh, hello world world"` and duplicated todo items. Folding a decoded model in LOOKS like a local edit; content-only equality saves the already-in-sync case (Step 6's L4 test) but not the catching-up case, which is every real `Set`. The fix is `Attachment.RunSuppressed`: during a fold-in the encode callbacks realign trackers and write nothing. Maps had silently survived only because per-key LWW restamps are idempotent — text and lists exposed the truth.
- The old `Program.fs` known-wrong assertions (`PropC`/`PropD`) died with their file rather than being fixed — nothing referenced them.
